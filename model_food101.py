...
def get_data_loaders(batch_size=BATCH_SIZE, noise=0.0) -> Tuple[DataLoader, DataLoader ]:
    ...
    num_labels = int(noise * len(train_data))
    random.seed(42)
...
def test_model(model, test_loader,criterion) -> Tuple[float, float]:
    """
    Test the model on the test set and returns the test loss and test accuracy.
    """
...
def train_model(model, train_loader, test_loader, criterion, optimizer, num_epochs, csv, start_epoch=0):
    """
    Main training loop.
    """
...
if __name__ == '__main__':
    ...
    model.to(device)
    train_loader, test_loader = get_data_loaders(noise=args.noise)
    train_model(model, train_loader, test_loader, criterion, optimizer, csv=epoch_stats_csv, num_epochs=NUM_EPOCHS)

    # save the model
    torch.save(model.state_dict(), f"./models/food101_resnet18_after_train_noise_{current_date}.pth")
